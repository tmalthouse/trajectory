...
/// node_vector returns the orbital node vector n given the angular momentum vector.
...
/// orb_inclination calculates the inclination given the angular momentum vector.
double orb_inclination(Vector3d h_vector);

/// orb_long_asc_node calculates the longitude of the ascending node given the n vector.
double orb_long_asc_node(Vector3d node_vector);

/// arg_of_periapsis uses the node and ecc vectors to calculate the argument of periapsis, one of Kepler's 5 elements.
double arg_of_periapsis(Vector3d node_vector, Vector3d ecc_vector);

/// true_anomaly calculates the true anomaly (angle between the periapsis and current location based off the center body)
double true_anomaly(Vector3d ecc_vector, Vector3d pos, Vector3d vel);

/// ecc_anomaly is like true anomaly, but the center of the angle is the center of the ellipse
double ecc_anomaly(double ecc, double true_ano);

/// The mean anomaly is like the true anomaly, but the body's position if it were in a circular orbit with equal period
double mean_anomaly(double ecc_ano, double ecc);

/// ecc_ano_from_mean_ano uses Newton's method to calculate the eccentric anomaly given the mean anomaly. This takes a bit longer than most functions here, so cache the result if it makes sense to.
double ecc_ano_from_mean_ano(double mean_ano, double ecc);

/// mean_ano_at_t calculates the mean anomaly of an orbit at a time T after the epoch
...
/// Uses a simple rotation to convert bodycentric coordinates to Cartesian coordinates, given an orbit.
