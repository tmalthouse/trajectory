...
static double calculate_ratio(Vector2d tr, Vector2d bl) {
  ...
  double lratio = (fabs(xratio) < fabs(yratio))
                      ? xratio
                      : yratio;  // Set the ratio to the smaller of the two.
  original_ratio = lratio;
  dblogger("Ratio is %f\n", lratio);
  return lratio;
}

void set_screensize(Vector2d size) { screensize = size; }

void scale_display(double factor) { ratio *= factor; }
...
void focus_body(Body *b) { focus = b; }
...
int item_pointed_at(Vector2d *coords, uint64_t count, Vector2d mousecoord,
                    float threshold) {
  int closest_obj = -1;
